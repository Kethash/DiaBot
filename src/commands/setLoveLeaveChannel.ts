...
export = {
    ...
    async execute(redisClient: any, interaction: ChatInputCommandInteraction<CacheType>) {
        ...
        if (!interaction.guild?.channels.cache.find((channel) => channel.id == newChannel)) {
            await interaction.reply({content: "BUU BUU !! Channel not found", flags: MessageFlags.Ephemeral});
            return;
        }
        ...
        try {
            ...
            await interaction.reply({content: "Couldn't set the new loveleave! channel. I'm sorry...", flags: MessageFlags.Ephemeral});
            return;
        }

        const newChannelName: string = interaction.guild.channels.cache.get(newChannel)?.toString() as string;
        await interaction.reply({content: `Loveleavechannel set on: ${newChannelName}`, flags: MessageFlags.Ephemeral});
